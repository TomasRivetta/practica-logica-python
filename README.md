# 🧠 Python Logic & Problem Solving

![Python](https://img.shields.io/badge/Python-3.12-blue?logo=python)
![Repo Size](https://img.shields.io/github/repo-size/TomasRivetta/practica-logica-python)
![Last Commit](https://img.shields.io/github/last-commit/TomasRivetta/practica-logica-python)
![Issues](https://img.shields.io/github/issues/TomasRivetta/practica-logica-python)
![Stars](https://img.shields.io/github/stars/TomasRivetta/practica-logica-python?style=social)
![Status](https://img.shields.io/badge/status-en%20progreso-yellow)

Repositorio dedicado a la práctica intensiva de lógica de programación utilizando Python.

---

## 🚀 Sobre este repositorio

Este repositorio está diseñado como un espacio de entrenamiento constante en **problem solving** y **pensamiento algorítmico**.

Cada ejercicio incluye:
- 📝 Enunciado del problema  
- 💡 Enfoque de resolución  
- 🧪 Implementación en Python  

---

## 📂 Estructura del proyecto

```bash
freeCodeCamp/        # Ejercicios diarios (freeCodeCamp)
adventJS2025/        # Problemas de AdventJS creados por midudev
adventJS2024/        # Problemas de AdventJS creados por midudev
adventJS2023/        # Problemas de AdventJS creados por midudev
adventJS2022/        # Problemas de AdventJS creados por midudev
adventJS2021/        # Problemas de AdventJS creados por midudev
```

---

📊 Progreso automático
<!-- START_PROGRESS -->
| Categoría | Completados | Total | Progreso |
|-----------|-------------|-------|----------|
| 🟢 freeCodeCamp | 1 | 203 | 0.5% |
| 🟡 AdventJS2025 | 1 | 25 | 4.0% |
| 🔴 AdventJS2024 | 0 | 25 | 0.0% |
| 🔵 AdventJS2023 | 0 | 25 | 0.0% |
| 🟣 AdventJS2022 | 0 | 25 | 0.0% |
| 🟠 AdventJS2021 | 0 | 25 | 0.0% |
| **Total general** | **2** | **328** | **0.6%** |
<!-- END_PROGRESS -->

---

## 🔗 Autor

### Tomás Rivetta

#### 🌐 Portfolio: https://totoridev.netlify.app/
#### 💻 GitHub: https://github.com/TomasRivetta
#### 🔗 LinkedIn: https://linkedin.com/in/tomasrivetta-fullstack/